# Setting up Python script for Abaqus
import csv
import os
from abaqus import *
...
def main():
    # Print the entire argument list (for debugging)
    print("sys.argv =", sys.argv)

    # Usually, Abaqus places script-related arguments last
    args = sys.argv
    # If you expect EXACTLY 6 items plus 5 numeric items, do a quick check
    if len(args) < 6:
        raise ValueError("Need at least 5 numeric args (R, L, t_s, t_h, t_n) at the end!")

    # Extract the last 5 arguments as strings:
    R_str, L_str, t_s_str, t_h_str, t_n_str = args[-5:]
    ...
    print("Parsed design variables:")
    print(" R={}, L={}, t_s={}, t_h={}, t_n={}".format(R, L, t_s, t_h, t_n))
    ...
    sketchPlaneSide=SIDE1, sketchOrientation=RIGHT, origin=(650.0, 625.0, 
    249.999854))
    ...
    job_name = 'ParamJob'
    my_job = mdb.Job(name=job_name, model='Model-1')
    ...
    if not os.path.exists(odb_path):
        ...
        last_frame = odb.steps['PressureStep'].frames[-1]
        ...
        pr = mdb.models['Model-1'].rootAssembly.getMassProperties()
        total_mass = pr['mass']
        odb.close()
        
    print("Max stress found: {:.6f} MPa".format(max_stress))
    print("Total mass found: {:.6f} tonnes".format(total_mass) if total_mass else "Mass data not available.")

    # Write results to file
    with open("results.txt", "w") as fout:
        fout.write("MAX_STRESS={:.6f}\n".format(max_stress))
        if total_mass is not None:
            fout.write("MASS={:.6f}\n".format(total_mass))

    print("Wrote MAX_STRESS and MASS to results.txt")
